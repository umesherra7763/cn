...
HOST = '172.16.204.222'  # Connect to localhost by default. CHANGE TO SERVER'S LAN IP
...
class MainConferencePage(QWidget):
    ...
    def stop_presenting(self):
        """Switch back to video grid view after screen sharing stops."""
        self.presenter_label.hide()
        self.screen_share_display.setText("No one is presenting.")
        self.screen_share_display.setPixmap(QPixmap())
        self.stage_stack.setCurrentWidget(self.video_grid_page)

        # Re-enable Share button for future use
        self.share_button.setDisabled(False)
        self.share_button.setToolTip("Share Screen")

        # ✅ Only uncheck if the user is not currently toggling it manually
...
class MainWindow(QMainWindow):
    ...
    def on_share_screen_toggled(self, checked):
        ...
        if checked:
            ...
            # The user *knows* they clicked stop.
            self.conference_page.stop_presenting()
